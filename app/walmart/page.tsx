export default function Walmart() {
  const deals = [
    {emoji:'📺', badge:'-61%', name:'Onn 55" 4K UHD Roku TV', price:'$118', orig:'$298', save:'$180', stock:'In stock nearby', stockColor:'#2a7a3b'},
    {emoji:'🧹', badge:'-63%', name:'Bissell PowerForce Vacuum', price:'$29', orig:'$79', save:'$50', stock:'In stock nearby', stockColor:'#2a7a3b'},
    {emoji:'🍳', badge:'-55%', name:'Pioneer Woman 24pc Cookware Set', price:'$49', orig:'$109', save:'$60', stock:'Only 4 left', stockColor:'#e67e00'},
    {emoji:'🪑', badge:'-70%', name:'Mainstays Outdoor Lounge Chair', price:'$18', orig:'$59', save:'$41', stock:'In stock nearby', stockColor:'#2a7a3b'},
    {emoji:'💊', badge:'-45%', name:'Equate Vitamin C 500ct', price:'$6', orig:'$11', save:'$5', stock:'In stock nearby', stockColor:'#2a7a3b'},
    {emoji:'🖨️', badge:'-58%', name:'HP DeskJet 2755e Printer', price:'$42', orig:'$99', save:'$57', stock:'Last 2', stockColor:'#e67e00'},
  ];
  ...
  return (
    <div style={{fontFamily: 'sans-serif', background: '#F7F5F0', minHeight: '100vh'}}>
      ...
      {/* RETAILER HEADER */}
      <div style={{background: '#0071CE', padding: '2rem 2rem 2.5rem'}}>
        <div style={{display: 'inline-block', background: 'rgba(255,255,255,0.15)', color: '#fff', borderRadius: '20px', padding: '4px 12px', fontSize: '11px', fontWeight: 500, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '0.75rem'}}>
          Clearance tracker
        </div>
        <h1 style={{fontSize: '32px', fontWeight: 800, color: '#fff', margin: '0 0 0.5rem', letterSpacing: '-1px'}}>
          Walmart Clearance
        </h1>
        <p style={{color: 'rgba(255,255,255,0.75)', fontSize: '14px', margin: '0 0 1.5rem'}}>
          {deals.length} clearance deals tracked today · Updated every 4 hours
        </p>
        <div style={{display: 'flex', gap: '1rem'}}>
          <a href="/locator?zip=" style={{background: '#fff', color: '#0071CE', borderRadius: '8px', padding: '9px 18px', fontSize: '13px', fontWeight: 600, textDecoration: 'none'}}>
            📍 Find near me
          </a>
          <div style={{background: 'rgba(255,255,255,0.15)', color: '#fff', borderRadius: '8px', padding: '9px 18px', fontSize: '13px', fontWeight: 500}}>
            83 items in stock locally
          </div>
        </div>
      </div>
      ...
      <div style={{background: '#fff', borderBottom: '1px solid #e8e4dc', padding: '0.75rem 2rem', display: 'flex', gap: '8px', overflowX: 'auto'}}>
        {['All','Electronics','Home','Kitchen','Outdoor','Health'].map((cat, i) => (
          <div key={cat} style={{background: i === 0 ? '#1a1a1a' : '#F7F5F0', color: i === 0 ? '#fff' : '#555', borderRadius: '6px', padding: '6px 14px', fontSize: '12px', fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap'}}>
            {cat}
          </div>
        ))}
      </div>
